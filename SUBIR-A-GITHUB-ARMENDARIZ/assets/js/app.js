(function () {
  ...
  const renderContact = () => {
    ...
    const items = [
      ["Teléfono", data.contact.phoneDisplay],
      ["WhatsApp", data.contact.whatsappDisplay],
      ["Dirección", data.contact.address],
  ...
  const setupForm = () => {
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (status) {
        status.textContent = "Se abrió WhatsApp con la solicitud lista para enviar.";
      }
    });
  };
  ...
  const setupSeo = () => {
    const title = `${data.doctor.name} | Consultorio odontológico`;
    const description = `${data.hero.title}. Turnos, servicios, dirección y contacto por WhatsApp.`;
