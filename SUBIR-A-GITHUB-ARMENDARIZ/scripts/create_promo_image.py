...
def draw_logo_header(canvas, draw, logo_size=(180, 180), box=(70, 54, 288, 230), text_x=324):
    ...
    draw.text((text_x, box[1] + 20), "Dr. Andrés Armendáriz", font=font(FONT_BOLD, 39), fill=INK)
    draw.text((text_x, box[1] + 70), "Odontología general", font=font(FONT_REGULAR, 28), fill=TEAL)
    draw.text((text_x, box[1] + 112), "Paraná 460 · Bahía Blanca", font=font(FONT_REGULAR, 26), fill=MUTED)
...
def create_square_general():
    ...
    y = draw_text(
        draw,
        (70, 330),
        "Atención odontológica profesional y personalizada",
        font(FONT_BLACK, 66),
        INK,
        520,
        line_gap=10,
    ...
    draw_text(
        draw,
        (72, y),
        "Turnos simples por WhatsApp y atención cercana para cuidar tu salud bucal.",
...
def create_story():
    ...
    draw.text((155, 914), "Consultorio en Bahía Blanca", font=font(FONT_BOLD, 34), fill=TEAL_DARK)

    y = draw_text(
        draw,
        (82, 1058),
        "Cuidá tu salud bucal con atención cercana",
        font(FONT_BLACK, 72),
        INK,
        900,
        line_gap=12,
    ...
    draw_text(
        draw,
        (86, y),
        "Turnos por WhatsApp para consultas generales, limpiezas, arreglos, ortopedia y urgencias odontológicas.",
...
def create_services_square():
    ...
    draw.text((318, 78), "Servicios odontológicos", font=font(FONT_BLACK, 52), fill=INK)
    draw.text((320, 144), "Dr. Andrés Armendáriz · Bahía Blanca", font=font(FONT_REGULAR, 29), fill=TEAL)
    ...
    draw.text((80, 1000), "Paraná 460 · Bahía Blanca", font=font(FONT_REGULAR, 27), fill=MUTED)
